...
Backend runtime остаётся неутверждённым до финализации `ADR-0001`; остальные позиции стека остаются неутверждёнными.
...
ADR-0001 — выбор языка и runtime backend, на ревью.

## 9. Следующий планируемый блок

Финализация `ADR-0001`, затем `ADR-0002` — backend API adapter.
