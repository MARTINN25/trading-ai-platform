...
3. результат — переключатель «Кратко»/«Подробно» (см. «Short/Full insight mode UI» ниже) над всеми 10 обязательными разделами FR-018, «Сгенерировано: ... · Источник анализа: AI», кнопка «Обновить AI-анализ» для повторной осознанной генерации;
...
#### Short/Full insight mode UI

Реализует FR-021 (краткий инсайт) / FR-022 (полный инсайт). **Presentation-only** — оба режима рендерят один и тот же уже полученный structured результат (`InstrumentAiAnalysis` для текущей генерации, `InsightDetail` для сохранённого инсайта); переключение **не отправляет ни одного HTTP-запроса**, не вызывает xAI, backend, market/news provider повторно, не создаёт новый инсайт и не трогает evaluation/outcome — только видимость уже загруженных полей меняется в браузере (проверено вживую через network log: 5 переключений подряд → 0 запросов).

Общий presentation-компонент `frontend/src/components/InsightSections.tsx` переиспользован и в `AiAnalysisSection` (текущая генерация), и в `InsightHistorySection` (сохранённый инсайт) — исключает дублирование разметки и гарантирует одинаковую семантику в обоих местах (задача явно требовала не оставлять ситуацию «текущий анализ умеет short/full, а история — только full»).

**Product Owner decision (через AskUserQuestion): сбалансированный состав краткого режима (5 из 10 разделов FR-018)** — «Краткий вывод», «Ключевые факты» (с источниками — FR-011 не теряется даже в кратком виде), «Уровень уверенности», «Что можно рассмотреть», «Основные риски». Только в полном режиме: «Анализ» (контекст цены/новостей), «Инсайт / гипотеза», «Что сильнее всего повлияло на вывод», «Актуальность данных». **Default — «Кратко»** (тоже Product Owner decision) — соответствует цели FR-021 «быстро увидеть суть».

Ни один раздел не «сжимается» текстом — короткий режим показывает реальный, уже сгенерированный текст выбранных разделов целиком, без клиентской обрезки и без серверной суммаризации (у документов не было основания для второй LLM-генерации, а обрезка готового текста исказила бы факт/обоснование).

Disclaimer («AI-анализ носит информационный характер...») показывается в обоих режимах всегда — переключатель на него не влияет.

Переключатель — семантические `<button>` с `aria-pressed`, доступны с клавиатуры (`focus-visible`), без новой UI-библиотеки. Режим хранится в локальном React state (`ADR-0003-frontend-stack.md` §23 явно относит короткий/полный режим к локальному UI-состоянию) — на `F5` сбрасывается к default («Кратко»), не персистится.
